...
* Ruin Town - As a mayor, deny your town to an invader using '/t delete'. Your town will enter a ruined state for 3 days, after which full deletion occurs. In this state, all perm protections are disabled. (Built directly into Towny now.)
* Reclaim Town - As a resident, if your town falls into ruin, then after 24 hrs. have passed, reclaim it and become mayor, using '/t reclaim'. (Built directly into Towny now.)
* Receive Nation Refund - As a king, if your nation gets deleted for any reason, including capture of your last town, you will be refunded(80%) of the initial setup cost. This can be claimed using '/n refund' or `sw nation refund` (for the `Towny-with-SiegeWar.jar` and `SiegeWar.jar` versions respectively.)

(Admin)
* Set siege immunities - As an admin, set siege immunities for 1 town, all towns in a nation, or all towns, using `/ta set siegeimmunities ...` or `/swa immunity` (for the `Towny-with-SiegeWar.jar` and `SiegeWar.jar` versions respectively.)
